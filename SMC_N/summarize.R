...
ggplot(logZsummary, aes(x=Particle, y=logZ, group = Method, colour=Method)) + 
  geom_errorbar(aes(ymin=logZ-1.96*sd, ymax=logZ+1.96*sd), width=.1) +
  geom_line() +
  geom_point()+ theme_bw()
dev.off()
...
p <- ggplot(ConsensusLogLLsummary, aes(x=Particle, y=Value, group = Adaptive, colour=Adaptive))
p2 <- ggplot(PartitionMetricsummary, aes(x=Particle, y=Value, group = Adaptive, colour=Adaptive))
p3 <- ggplot(RobinsonFouldsMetricsummary, aes(x=Particle, y=Value, group = Adaptive, colour=Adaptive))
p4 <- ggplot(KuhnerFelsensteinsummary, aes(x=Particle, y=Value, group = Adaptive, colour=Adaptive))

gname = c("SMCParticle.eps",sep="")  
postscript(gname,width=10,height=3,horizontal = FALSE, onefile = FALSE, paper = "special")
par(mfrow=c(1,1),oma=c(0.2,1.5,0.2,1.5),mar=c(3,2,0.2,2),cex.axis=1,las=1,mgp=c(1,0.5,0),adj=0.5)

ggarrange(ggplot(logZsummary, aes(x=Particle, y=logZ, group = Method, colour=Method)) + 
            geom_errorbar(aes(ymin=logZ-1.96*sd, ymax=logZ+1.96*sd), width=.3) +
            geom_line() +rremove("ylab")+ geom_point()+ xlab('logZ')+ theme_bw()+ylab(NULL) + theme(
              axis.text.x = element_blank()),
          p +  geom_errorbar(aes(ymin=Value-1.96*sd, ymax=Value+1.96*sd), width=.3) +
            geom_line() +
            geom_point()+rremove("ylab")+rremove("legend")+ xlab('ConsensusLogLL')+ theme_bw()+ylab(NULL) + theme(
              axis.text.x = element_blank())
          ,p2 +  geom_errorbar(aes(ymin=max(0, Value-1.96*sd), ymax=Value+1.96*sd), width=.3) +
            geom_line() + geom_point()+rremove("ylab") + xlab('PartitionMetric')+ theme_bw()+ylab(NULL) + theme(
              axis.text.x = element_blank())
          ,p3 +  geom_errorbar(aes(ymin=Value-1.96*sd, ymax=Value+1.96*sd), width=.3) +
            geom_line() + geom_point()+rremove("ylab") + xlab('RobinsonFouldsMetric')+ theme_bw()+ylab(NULL) + theme(
              axis.text.x = element_blank())
          ,p4 +  geom_errorbar(aes(ymin=max(0, Value-1.96*sd), ymax=Value+1.96*sd), width=.3) +
