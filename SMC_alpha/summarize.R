...
ggplot(logZsummary, aes(x=alpha, y=logZ, group = Method, colour=Method)) + 
  geom_errorbar(aes(ymin=logZ-sd, ymax=logZ+sd), width=.1) +
  geom_line() +
  geom_point()+ theme_bw()
dev.off()
...
ggarrange(ggplot(logZsummary, aes(x=alpha, y=logZ, group = Method, colour=Method)) + 
          ...
          ,p4 +  geom_errorbar(aes(ymin=max(0, Value-1.96*sd), ymax=Value+1.96*sd), width=.3) +
            geom_line() + geom_point()+rremove("ylab") + xlab('KuhnerFelsenstein')+ theme_bw()+ylab(NULL) + theme(
              axis.text.x = element_blank())
          ,p5 +  geom_errorbar(aes(ymin=T-1.96*sd, ymax=T+1.96*sd), width=.3) +
