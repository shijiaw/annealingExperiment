...
process createPlot {
  ...
  input:    
   ...
   output:
    ...
    file 'relaxedClockTreeDistanceASMCvsCSMC.eps'
    file 'relaxedclockResults.csv'
  ...
  Method <-  rep("CSMC-Nonclock", nrow(results))
  Method[results[,2]=="ANNEALING TRUE"] <- " ASMC"
  Method[results[,2]=="ANNEALING FALSE"] <- " DASMC"
  
  results2 <- data.frame(results,Method)
  write.csv(results2, "relaxedclockResults.csv")
  results2 <- results2[-which(Method==" DASMC"),]

  head(results2)
  postscript("relaxedClockTreeDistanceASMCvsCSMC.eps",width=10,height=4,horizontal = FALSE, onefile = FALSE, paper = "special")
