...
process buildCode {
  ...
  input:
    ...
    val codeRevision from '10d58f7be022d4cdae4689352594ae3be9c4bd86'
...
process run {
  ...
  input:
    file code   
     ...
     each simNonclockTree from (true,false)
    echo true
  ...
  mkdir state
  mkdir state/execs
  java -cp code/lib/\\* -Xmx2g smcsampler.SMCSamplerExperiments   \
     ...
     -useNonclock $simNonclockTree  \
     -useSlightNonclock false \
     -iterScalings  $particle    100000 \
     -methods      ANNEALING CSMC  \
...
process createPlot {
  ...
  input:    
   ...
   output:
    ...
    file 'treeDistanceSimuNonclocktrue.eps'
    file 'treeDistanceSimuNonclockfalse.eps'
  ...
  results2 <- results[results[,'gen.useNonclock'] == 'true',]  
  postscript("treeDistanceSimuNonclocktrue.eps",width=10,height=4,horizontal = FALSE, onefile = FALSE, paper = "special")
  ...
  results2 <- results[results[,'gen.useNonclock'] == 'false',]  
  postscript("treeDistanceSimuNonclockfalse.eps",width=10,height=4,horizontal = FALSE, onefile = FALSE, paper = "special")
