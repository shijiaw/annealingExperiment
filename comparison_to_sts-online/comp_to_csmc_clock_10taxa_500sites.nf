...
process createPlot {
  ...
  input:    
   ...
   output:
    ...
    file 'clockTreeDistanceASMCvsCSMC.eps'
    file 'clockResults.csv'
  ...
  write.csv(results2, "clockResults.csv")
  results2 <- results2[-which(Method==" DASMC"),]
  head(results2)
  postscript("clockTreeDistanceASMCvsCSMC.eps",width=10,height=4,horizontal = FALSE, onefile = FALSE, paper = "special")
