...
echo true
  ...
  input:
    ...
    each mainRand from  452
    each rand from  345
    each alphaSMCSampler from 0.9999
    each nSitesPerIndex from (1, 10, 100, 1000, 6000)
    echo true
...
java   -cp 'code/lib/*' -Xmx16G   smcsampler.SMCSamplerExperiments -useDataGenerator true    -nThousandIters 0.001 \
     -nTax 10  \
     -len  6000 \
     -sequenceType DNA \
     ...
     -iterScalings    100  100 \
     -methods    ANNEALING  SUBSAMPLING  \
     -resamplingStrategy  ESS  \
     -nAnnealing 50000  \
     -alphaSMCSampler $alphaSMCSampler \
     -nSitesPerIndex  $nSitesPerIndex\
